...
cg.county_id = wf.county_id 
AND wh.data_block_id = wf.data_block_id 
AND wh.latitude=wf.latitude AND wh.longitude=wf.longitude
AND EXTRACT(HOUR FROM wh.datetime) = EXTRACT(HOUR FROM wf.forecast_datetime)
...
AND TO_DATE(ep.forecast_date)=gp.forecast_date
...
cw.data_block_id = ce.data_block_id AND
cw.wh_observ_4hour=ce.t_observ_4hour